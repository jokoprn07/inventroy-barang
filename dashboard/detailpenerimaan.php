...
$barang = mysqli_query($conn, "SELECT * FROM `dp_barang` WHERE kode_pembelian = '$kode' ");
...
<nav class="navbar-default navbar-side" role="navigation">
    ...
    <div id="page-wrapper">
      ...
      <div class="row">
        <div class="col-12">
            <h3 style="margin-left: 20px;"><i class="fas fa-cube"></i> Data Penerimaan Barang</h3>
            <hr>
        </div>
      </div>
    ...
    <div class="table-responsive">
      <table class="table table-striped table-bordered">
        ...
        <?php foreach($barang as $data) : ?>
            ...
            <td><?= $data['ppn']; ?> % </td>
            <td><?= $data['jumlah_terima'] == 0 ? 'Belum Menerima Barang' : $data['jumlah_terima']; ?></td>
            <td><?= $data['divisi']; ?></td>
            <td><?= $data['keterangan']; ?></td>
            <td><a href="editpenerimaan.php?id=<?= $data['id']; ?>"><button class="btn btn-success"><i class="fas fa-edit"></i> Proses</button></a>
            <a href="hapuspenerimaan.php?id=<?= $data['id']; ?>" onclick="return confirm('Apakah Anda Yakin Ingin Menghapus Data Ini?');"><button class="btn btn-danger"><i class="fas fa-trash"></i> Hapus</button></a>
            </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
