...
$barang = mysqli_query($conn, "SELECT `kode_pembelian`,`nama_barang`,`satuan`,`harga_beli`, SUM(`jumlah_pembelian`) AS stock FROM dp_barang GROUP BY `nama_barang` ASC LIMIT $awalData, $jumlahDataPerHalaman");

if( isset($_POST['search']) ){
  $barang = cari(mysqli_real_escape_string($conn, $_POST['keyword']));
...
<nav class="navbar-default navbar-side" role="navigation">
      <div class="sidebar-collapse">
        <ul class="nav" id="main-menu">
          ...
          <li>
            <a  href="#"><i class="fas fa-cube"></i>  Penerimaan<span class="fa arrow"></span></a>
            <ul class="nav nav-second-level">
              <li>
    ...
    <div id="page-wrapper">
      ...
      <a href="tambah.php"><button class="btn btn-success"><i class="fa fa-plus-square"></i> Tambah Data</button></a>

      <div class="row">
        <div class="col-12">
            <h3 style="margin-left: 20px;"><i class="fas fa-cube"></i> Data Stock Karton</h3>
            <hr>
        </div>
      </div>
    ...
    <div class="table-responsive">
      <table class="table table-striped table-bordered">
        ...
        <?php foreach($barang as $data) : ?>
            ...
            <td><a href="edit.php?id=<?= $data['id']; ?>"><button class="btn btn-primary"><i class="fas fa-edit"></i> Edit</button></a> | 
            <a href="hapus.php?id=<?= $data['id']; ?>" onclick="return confirm('Apakah Anda Yakin Ingin Menghapus Data Ini?');"><button class="btn btn-danger" onclick="hapusBarang()"><i class="fas fa-trash"></i> Hapus</button></a>
            </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
