...
$suplier = query( "SELECT * FROM suplier WHERE id = $id")[0];

if ( isset($_POST["submit"]) ) {
	if ( editsupp($_POST) > 0 ) {    
    echo "<script>alert('Data Suplier Berhasil Diubah');
    window.location.href = 'suplier.php';
    </script>";
	} else {
		echo "<script>alert('Data Suplier Gagal Diubah')</script>";
	}
}
...
<nav class="navbar-default navbar-side" role="navigation">
    ...
    <div id="page-wrapper">
      ...
      <div class="row">
        <div class="col-12">
            <h3 style="margin-left: 20px;"><i class="fas fa-plus-square"></i> Ubah Data Suplier</h3>
            <hr>
        </div>
      </div>
        ...
        <div class="row">
            ...
            <input type="hidden" name="id" id="id" value="<?= $suplier['id']; ?>" >
            <div class="form-group col-md-6">
                <label for="kode">Kode Suplier :</label>
                <input type="text" id="kode" name="kode_suplier" class="form-control" placeholder="Masukkan Kode Customer" autocomplete="off" value="<?= $suplier['kode_suplier'] ?>">
            </div>
            <div class="form-group col-md-6">
                <label for="item">Nama Suplier :</label>
                <input type="text" id="item" name="nama_suplier" class="form-control" placeholder="Masukkan Nama suplier" autocomplete="off" value="<?= $suplier['nama_suplier'] ?>">
            </div>
            <div class="form-group col-md-6">
                <label for="alamat">Alamat Suplier :</label>
                <input type="text" id="alamat" name="alamat_suplier" class="form-control" placeholder="Masukkan Alamat suplier" autocomplete="off" value="<?= $suplier['alamat_suplier'] ?>">
            </div>
            <button type="submit" name="submit" class="btn btn-primary form-control">Edit Data Admin</button>
            </form>
            </div>
        </div>
