...
if( isset($_POST['add_non']) ){
  if( tambahPembelianKarton($_POST) > 0 ){
    Header('Location: tambahpembeliannonkarton.php');
    exit();
  }else{
...
<nav class="navbar-default navbar-side" role="navigation">
    ...
    <div id="page-wrapper">
      ...
      <div class="row">
        ...
        <div class="row">
            <div class="col-md-12">
            <form action="tambahpembeliannonkarton.php" method="post">
                <div class="form-group col-md-6">
                    ...
                    <select name="nama_customer" id="nama_customer" data-live-search="true" class="form-control selectpicker" onchange="cek_cst()" >
                        ...
                        <option value="<?= $cst['nama_customer'] ?>"><?= $cst['nama_customer'] ?> - <?= $cst['kode_customer'] ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group col-md-6">
                    <label for="nama_suplier">Nama Suplier :</label>
                    <select name="nama_suplier" id="nama_suplier" data-live-search="true" class="form-control selectpicker" onchange="cek_database()" >
                        <option value="">-- Pilih Nama Suplier --</option>
                        <?php 
                          $suplier = mysqli_query($conn, "SELECT * FROM suplier");
                        ?>
                        <?php foreach( $suplier as $supp ) : ?>
                        <option value="<?= $supp ['nama_suplier'] ?>"><?= $supp ['nama_suplier'] ?> - <?= $supp['kode_suplier']; ?> </option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group col-md-6">
                    <label for="kode_customer">Kode Customer :</label>
                    <input type="text" id="kode_customer" readonly required name="kode_customer" class="form-control" placeholder="Masukkan Kode Customer" autocomplete="off">
                </div>
                <div class="form-group col-md-6">
                    <label for="kode_suplier">Kode Suplier :</label>
                    <input type="text" id="kode_suplier" readonly required name="kode_suplier" class="form-control" placeholder="Masukkan Kode Suplier" autocomplete="off">
                </div>
                <div class="form-group col-md-6">
                  <label for="date">Tanggal Pembelian : </label>
                  <input required placeholder="Masukkan Tanggal Pembelian" type="date" id="date" class="form-control" name="tanggal_pembelian" autocomplete="off">
                </div>
                <div class="form-group col-md-6">
                  <label for="datepicker">Jadwal Kirim : </label>
                  <input required placeholder="Masukkan Jangka Waktu Pembelian" type="date" id="datepicker" class= "form-control" name="jangka_waktu" autocomplete="off">
                </div>
                <div class="form-group col-md-6">
                    <label for="kode_pembelian">No PO :</label>
                    <input type="text" id="kode_pembelian" required name="kode_pembelian" class="form-control" placeholder="Masukkan Kode Pembelian" autocomplete="off">
                </div>
                <button id="add" type="submit" name="add_non" class="btn btn-primary form-control">Tambah Data</button>
